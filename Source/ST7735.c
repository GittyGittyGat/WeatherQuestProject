...
void ST7735_DrawBitmap(int16_t x, int16_t y, const uint16_t *image, int16_t w, int16_t h, uint16_t back_ground){
  ...
  for(y=0; y<h; y=y+1){
    for(x=0; x<w; x=x+1){
			if(image[i] == 0xFFFF){ //if we are on a background pixel (white)
					writedata((uint8_t)(back_ground >> 8));//change the color ot the background
					writedata((uint8_t)back_ground);
			}
      else{                               // send the top 8 bits
